...
class playGame extends Phaser.Scene {
    ...
    create() {
        ...
        /* this.socket.on('currentPlayers', function (playersIcon) {
            Object.keys(playersIcon).forEach(function (id) {
                if (playersIcon[id].playerId === self.socket.id) {
                    addPlayer(self, playersIcon[id]);
                } 
                else {
                    addOtherPlayers(self, playersIcon[id]);
                }
            });
        });
        ...
        this.socket.on("cardDrop", function (cardData) {
            ...
            let card = self.add.sprite(300 + 60 * cardData.x, 75 + 60 * cardData.y, cardData.value).setDepth(2);
    ...
    addPlayer(self, playerInfo) {
        self.ship = self.add.image(playerInfo.x, playerInfo.y, "player" + playerInfo.playerImage).setOrigin(0.5, 0.5).setDepth(1);
    }

    addOtherPlayers(self, playerInfo) {
        const otherPlayer = self.add.sprite(playerInfo.x, playerInfo.y, "player" + playerInfo.playerImgae).setOrigin(0.5, 0.5).setDepth(1);
