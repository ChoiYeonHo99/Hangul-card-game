...
class playGame extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("deck", "cards/뒷면.png");
    ...
    create() {
        ...
        this.socket.on("centerCard", function (value) {
            if(firstCenter) {
                ...
                let card = self.add.sprite(300 + 60 * 4, 75 + 60 * 4, value).setDepth(2).setInteractive().on("pointerdown",() => {
                    if(self.boardGroup.contains(card) & !card.activate & self.myTurn) {
                        console.log("카드 선택");
                        card.alpha = 0.5;
                        card.activate = true;
        ...
        let redbutton = this.add.sprite(950, 100, "redbutton").setInteractive().on("pointerup",() => {
            if (this.myTurn) {
                this.sortWord();
                this.socket.emit("turnEnd", this.socket.id, this.word);
    ...
    createCard(n) {
        ...
        let card = this.add.sprite(coordinates.x, coordinates.y, n).on("pointerdown",(pointer) => {
            if(pointer.rightButtonDown()) {
                ...
                if(this.boardGroup.contains(card) & !card.activate & this.myTurn) {
                    console.log("카드 선택");
                    card.alpha = 0.5;
                    card.activate = true;
                    this.alphaCard.push(card);
