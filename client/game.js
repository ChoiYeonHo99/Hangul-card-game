...
class playGame extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("okButton", "image/okButton.png");
        this.load.image("verificationTrueBox", "image/verificationTrueBox.png");
        this.load.image("verificationFalseBox", "image/verificationFalseBox.png");
        this.load.image("rightArrange", "image/rightArrange.png");
        this.load.image("leftArrange", "image/leftArrange.png");
        ...
        this.load.image("믄", "cards/믄.png");
        this.load.image("무", "cards/무.png");
        this.load.image("여", "cards/여.png");
        this.load.image("믕", "cards/믕.png");
        this.load.image("으", "cards/으.png");
    }
    create() {
        this.socket = io();
        var self = this;
        ...
        this.socket.on("verificationFalse", function (id) {
            ...
            for(var i = 0; i < 10; i++) {
                ...
                if (id === self.socket.id) {
                    ...
                    // 현재 카드 수를 플레이어에게 알린다
                    self.socket.emit("currentCardUpdate", self.socket.id, self.handGroup.countActive());
                }
            }
        ...
        this.socket.on("verificationTrue", function (id, meaning) {
        ...
        this.socket.on("timeOut", function () {
            ...
            if(self.myTurn) {
                ...
                self.arrangeCardsInHand()
                // turn을 종료한다
                self.socket.emit("turnEnd", self.socket.id, self.word, "time");
                self.socket.emit("nextTurn", null);
                self.myTurn = false;
        ...
        this.socket.on("turnPlayerDisconnection", function () {
            // drop한 card를 모두 제거한다
            self.waitBlankGroup.clear(true);
            self.waitCardGroup.clear(true);
            for(var i = 0; i < 10; i++) {
        ...
        this.socket.on("newPlayer", function (id, number) {
            if (number === 0) {
                ...
                self.player2currentCard = self.add.text(200, 360 + 210 * number, gameOptions.startingCards, currentCardStyle);
            }
            else if (number === 2) {
                self.player3Box = self.add.sprite(200, 340 + 210 * number, "player3");
                self.player3Name = self.add.text(200, 340 + 210 * number, id);
                self.player3currentCard = self.add.text(200, 360 + 210 * number, gameOptions.startingCards, currentCardStyle);
            }
            else if (number === 3) {
                self.player4Box = self.add.sprite(200, 340 + 210 * number, "player4");
                self.player4Name = self.add.text(200, 340 + 210 * number, id);
                self.player4currentCard = self.add.text(200, 360 + 210 * number, gameOptions.startingCards, currentCardStyle);
            }
        });
        ...
        this.socket.on("currentPlayers", function (id, number) {
            for (var i = 0; i < number; i++) {
                if (i === 0) {
                    self.player1Box = self.add.sprite(200, 340, "player1");
                    self.player1Name = self.add.text(200, 340, id[i]);
                    self.player1currentCard = self.add.text(200, 360, gameOptions.startingCards, currentCardStyle);
                }
                else if (i === 1) {
        ...
        this.socket.on("currentCardUpdate", function (id, number) {
            if (self.player1Name.text === id) {
        ...
        this.socket.on("tok", function (time) {
            var seconds = time;
        ...
        this.verificationTrueBox = self.add.sprite(1050, 500, "verificationTrueBox");
        this.verificationTrueBox.setDepth(5);
        this.verificationTrueBox.visible = false;
        this.verificationFalseBox = self.add.sprite(1050, 500, "verificationFalseBox");
        ...
        let deck = this.add.sprite(1810, 490, "deck").setInteractive().on("pointerup",() => {
            ...
            if(this.myTurn & !(this.dropped)) {
                ...
                console.log("whetherObjection: " + whetherObjection);
                // 현재 카드 수를 플레이어에게 알린다
                this.socket.emit("currentCardUpdate", this.socket.id, this.handGroup.countActive());
                this.timer.paused = true;
            }
        });
        deck.setDepth(1);
    ...
    tick() {
        ...
        this.socket.emit("tick", seconds);
        //check if the time is up
        if (this.timeInSeconds === 0 && this.myTurn) {
            this.timer.paused = true;
            this.socket.emit("timeOut", null);
        }
    }
