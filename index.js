...
io.on('connection', function (socket) {
    ...
    if (players.length === 1) {
        ...
        centerCardValue = Math.floor(Math.random() * 48);
        io.emit('centerCard', centerCardValue);
        console.log(centerCardValue);
    }
    else {
        io.emit('centerCard', centerCardValue);
    }
    ...
    socket.on('nextTurn', function () {
        do {
            turnCount++;
            currentTurn = players[(turnCount % playerCount)];
    ...
    socket.on('disconnect', function () {
        ...
        if (currentTurn === socket.id) {
            ...
            socket.broadcast.emit('turnPlayerDisconnection');
        }
    ...
    socket.on('tick', function (time) {
        socket.broadcast.emit('tok', time);
    });
